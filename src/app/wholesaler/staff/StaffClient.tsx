...
import {
  ...
  Trash2, Key, Edit, X, ToggleLeft, ToggleRight
} from 'lucide-react';
...
export default function StaffClient({ initialStaff, initialLogs }: StaffClientProps) {
  ...
  const handleCreateStaff = async (e: React.FormEvent) => {
    ...
    setError('');
    setSuccessMsg('');
    if (!email || !password || !fullName) { setError('Please fill in all employee credentials.'); return; }
    if (allowedFeatures.length === 0) { setError('Please select at least one allowed feature.'); return; }
    try {
  ...
  const handleUpdateStaff = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/wholesaler/staff/${editingStaff.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: editEmail, fullName: editFullName, password: editPassword || undefined, isActive: editIsActive, allowedFeatures: editAllowedFeatures }),
      });
      const data = await res.json();
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ fontSize: 13, color: '#64748B', marginTop: 4 }}>
            Manage staff login details, configure feature permissions, and review credentials.
          </p>
        </div>
        <button onClick={refreshData} className="btn-ghost" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '340px 1fr', gap: 16, alignItems: 'start' }}>
        ...
        <div className="card" style={{ background: 'rgba(255,255,255,0.85)', padding: 24 }}>
          ...
          <form onSubmit={handleCreateStaff} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              ...
              <input type="text" required placeholder="e.g. Ram Bahadur" value={fullName} onChange={e => setFullName(e.target.value)} className="input-crisp" />
            </div>
            <div>
              ...
              <input type="email" required placeholder="e.g. ram@distributor.com" value={email} onChange={e => setEmail(e.target.value)} className="input-crisp" />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 10, fontWeight: 700, color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>Password (Visible)</label>
              <input type="text" required placeholder="Create a strong password" value={password} onChange={e => setPassword(e.target.value)} className="input-crisp" style={{ fontFamily: 'monospace' }} />
            </div>

            {/* Permissions */}
            <div style={{ borderTop: '1px solid #F1F5F9', paddingTop: 14 }}>
              <label style={{ display: 'block', fontSize: 10, fontWeight: 700, color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 10 }}>Allowed Features</label>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4, background: '#F8FAFC', border: '1.5px solid #E0F2FE', borderRadius: 10, padding: '8px 10px' }}>
                {AVAILABLE_FEATURES.map((f) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span style={{ fontSize: 12, fontWeight: 600, color: isChecked ? '#1E293B' : '#64748B' }}>{f.label}</span>
                    </button>
                  );
                })}
              </div>
            ...
            <button type="submit" className="btn-primary" style={{ justifyContent: 'center', width: '100%', padding: '11px' }}>
        ...
        <div className="card" style={{ background: 'rgba(255,255,255,0.85)', padding: 24 }}>
          ...
          {staff.length === 0 ? (
            <div style={{ padding: '32px', textAlign: 'center', color: '#94A3B8', fontStyle: 'italic', fontSize: 12 }}>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, background: 'rgba(15,23,42,0.45)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }} className="no-print">
          ...
          >
            ...
            <form onSubmit={handleUpdateStaff} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: 10, fontWeight: 700, color: '#64748B', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>Email</label>
              ...
              <div style={{ display: 'flex', gap: 10, paddingTop: 8, borderTop: '1px solid #F1F5F9' }}>
                <button type="submit" className="btn-primary" style={{ flex: 1, justifyContent: 'center', padding: '12px' }}>
                  Save Changes
                </button>
