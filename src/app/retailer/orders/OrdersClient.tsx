...
export default function OrdersClient({ initialOrders, wholesalers, profileId, initialConsumerOrders }: OrdersClientProps) {
  ...
  const handlePlaceOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/retailer/orders', {
        ...
        body: JSON.stringify({ wholesalerId: selectedWholesalerId, items: cart.map(c => ({ productId: c.productId, qtyBoxes: c.qtyBoxes })), overrideJustification: needsOverride ? overrideMsg : undefined }),
      });
      const data = await res.json();
      if (!res.ok) {
        if (data.needsOverride) { setNeedsOverride(true); setOrderMessage({ text: data.error || 'Requires credit limit override note.', isError: true }); }
        else setOrderMessage({ text: data.error || 'Failed to place order.', isError: true });
        return;
      }
      if (data.success) {
  ...
  const addToCart = (product: Product, qtyVal: string) => {
    ...
    if (qty <= 0) { setCart(prev => prev.filter(c => c.productId !== product.id)); return; }
    const tpb = product.tabletsPerStrip * product.stripsPerBox;
    const totalUnits = product.batches.reduce((s, b) => s + b.availableBaseUnits, 0);
    const availableBoxes = Math.floor(totalUnits / tpb);
    if (qty > availableBoxes) { alert(`Max available: ${availableBoxes} boxes`); return; }
    const price = product.batches[0]?.sellingPricePerBox || 100;
    const item = { productId: product.id, name: product.name, qtyBoxes: qty, pricePerBox: price, availableBoxes };
    setCart(prev => { const idx = prev.findIndex(c => c.productId === product.id); if (idx > -1) { const copy = [...prev]; copy[idx] = item; return copy; } return [...prev, item]; });
  };
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                        ...
                        <tbody>
                          {selectedWholesaler.products.map(prod => {
                            ...
                            const totalUnits = prod.batches.reduce((s, b) => s + b.availableBaseUnits, 0);
                            ...
                            return (
                              <tr key={prod.id} style={{ borderBottom: '1px solid var(--card-border)', background: cartItem ? 'var(--table-header-bg)' : '' }}>
                                ...
                                <td style={{ padding: '10px 16px', textAlign: 'right', fontWeight: 700, color: 'var(--text-primary)', fontFamily: 'monospace' }}>Rs. {price}</td>
                                <td style={{ padding: '10px 16px', textAlign: 'center' }}>
                                  <input id={`qty-${prod.id}`} type="number" min="1" max={totalBoxes} placeholder="0" defaultValue={cartItem ? String(cartItem.qtyBoxes) : ''} style={{ ...inputStyle, textAlign: 'center', width: 80, padding: '6px 8px' }} />
                                </td>
                                <td style={{ padding: '10px 16px', textAlign: 'center' }}>
                                  <button onClick={() => addToCart(prod, (document.getElementById(`qty-${prod.id}`) as HTMLInputElement)?.value || '0')} style={{ padding: '6px 12px', borderRadius: 5, border: 'none', background: cartItem ? '#059669' : 'var(--text-primary)', color: 'var(--card-bg)', fontSize: 11, fontWeight: 700, cursor: 'pointer' }}>
                                    {cartItem ? 'Added' : 'Add'}
                                  </button>
                                </td>
                              </tr>
                  ...
                  {orders.filter(o => o.status === 'DISPATCHED').map(order => (
                    <div key={order.id} style={{ padding: '12px 16px', borderBottom: '1px solid var(--card-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        ...
                        <div style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: 'monospace' }}>ORD-{order.id.substring(0, 12).toUpperCase()}</div>
                      </div>
