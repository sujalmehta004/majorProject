...
export default function SettingsClient({
  ...
  const [activeTab, setActiveTab] = useState<'profile' | 'staff' | 'security' | 'logs' | 'theme'>('profile');
  ...
  // Whether fields are editable (excluding phone which is always editable)
  const fieldsLocked = (isPending || isVerified) && !isEditingVerifiedDetails;
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      setThemeMode(saved);
      const savedScale = localStorage.getItem('font_scale') as 'sm' | 'md' | 'lg' || 'md';
      ...
      const storedBoxes = localStorage.getItem('retailer_low_stock_boxes');
      if (storedBoxes) setLowStockBoxes(parseInt(storedBoxes, 10));
      const storedStrips = localStorage.getItem('retailer_low_stock_strips');
      if (storedStrips) setLowStockStrips(parseInt(storedStrips, 10));
      const storedTablets = localStorage.getItem('retailer_low_stock_tablets');
      ...
      if (storedFields) { try { setCustomFields(JSON.parse(storedFields)); } catch (e) {} }

      const params = new URLSearchParams(window.location.search);
      const tabVal = params.get('tab');
      if (tabVal === 'staff') setActiveTab('staff');
      else if (tabVal === 'logs') setActiveTab('logs');
      else if (tabVal === 'security') setActiveTab('security');
      const searchParam = params.get('search') || params.get('q');
      if (searchParam) { setSearchTerm(searchParam); setActiveTab('logs'); }
    }
  }, [initialProfile.id]);

  useRealtimeEvent('SETTINGS_UPDATE', () => { });
  useRealtimeEvent('STAFF_UPDATE', () => { fetchStaff(); });
  ...
  // Save phone only (no re-verification)
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg('Profile & registration documents submitted for Superadmin verification. ✓');
      if (typeof window !== 'undefined') {
  ...
  const handleSaveAlerts = () => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('retailer_low_stock_boxes', lowStockBoxes.toString());
      localStorage.setItem('retailer_low_stock_strips', lowStockStrips.toString());
      localStorage.setItem('retailer_low_stock_tablets', lowStockTablets.toString());
      localStorage.setItem('retailer_expiry_alert_days', expiryAlertDays.toString());
      setSuccessMsg('Alert threshold settings saved.');
    }
  };
  ...
  const handleOpenCreateStaff = () => {
    ...
    setStaffFeatures(AVAILABLE_FEATURES); setStaffIsActive(true); setError(''); setSuccessMsg('');
  ...
  const handleDeleteStaff = async (id: string, name: string) => {
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) { setSuccessMsg(`Account "${name}" deleted.`); broadcastUpdate('STAFF_UPDATE'); }
      else setError(data.error || 'Failed to delete account');
    } catch (err: any) { setError(err.message || 'Error deleting account'); }
  };

  const toggleFeature = (feature: string) => {
    setStaffFeatures((prev) => prev.includes(feature) ? prev.filter((f) => f !== feature) : [...prev, feature]);
  };
  ...
  const Modal = ({ children, onClose, title, maxWidth = 480 }: { children: React.ReactNode; onClose: () => void; title: string; maxWidth?: number }) => (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 300, background: 'rgba(15,23,42,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth, background: 'var(--card-bg)', borderRadius: 14, border: '1px solid var(--card-border)', display: 'flex', flexDirection: 'column', maxHeight: '92vh', overflow: 'hidden', boxShadow: '0 24px 48px rgba(0,0,0,0.2)' }}>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, minHeight: '100vh', width: '100%' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 12, borderBottom: '1px solid var(--card-border)', paddingBottom: 16 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--text-secondary)', margin: '2px 0 0' }}>Configure profile details, manage employees, adjust alert thresholds, and audit activity logs.</p>
        </div>
      ...
      <div style={{ display: 'flex', gap: 6, background: 'var(--table-header-bg)', padding: 4, borderRadius: 8, border: '1px solid var(--card-border)', flexWrap: 'wrap' }}>
        {([
          { id: 'profile', label: '🏪 Profile & Registration' },
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {isVerified && !isEditingVerifiedDetails && (
            <div style={{ background: 'linear-gradient(135deg,#F0FDF4,#ECFDF5)', border: '1.5px solid #86EFAC', borderRadius: 12, padding: '14px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  <div style={{ fontSize: 13, fontWeight: 800, color: '#065F46' }}>Account Verified & Active</div>
                  <div style={{ fontSize: 12, color: '#059669', marginTop: 2 }}>Profile is locked. Phone number can be updated anytime. To edit other details, click <strong>Unlock & Re-verify</strong>.</div>
                </div>
              </div>
          ...
          {isVerified && isEditingVerifiedDetails && (
            <div style={{ background: '#FFFBEB', border: '1.5px solid #FDE68A', borderRadius: 12, padding: '12px 18px', fontSize: 12, color: '#92400E', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <span>⚠ <strong>Edit Mode Active:</strong> Saving changes to profile details or documents will revert your verification status to <strong>PENDING</strong> for Superadmin re-review.</span>
            </div>
          )}
          {isPending && (
            <div style={{ background: 'linear-gradient(135deg,#FFFBEB,#FEF9C3)', border: '1.5px solid #FDE68A', borderRadius: 12, padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 12, color: '#B45309', marginTop: 2 }}>Superadmin is reviewing your registration documents. Profile fields are locked. You can still update your <strong>phone number</strong> below.</div>
              </div>
            </div>
          )}
          {isRejected && (
            <div style={{ background: '#FEF2F2', border: '1.5px solid #FECACA', borderRadius: 12, padding: '14px 20px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: '#DC2626', marginTop: 2 }}>Please correct the issues below and resubmit for verification.</div>
                </div>
              </div>
          ...
          <div style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 12, padding: 18 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>Store Phone Number</span>
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
              <input type="text" value={phone} onChange={(e) => setPhone(e.target.value)} style={{ ...inputStyle, flex: 1 }} placeholder="e.g. 98xxxxxxxx" />
          ...
          <form onSubmit={handleUpdateProfile} style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 12, padding: 20, display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4, flexWrap: 'wrap', gap: 8 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <Building style={{ width: 16, height: 16, color: '#F59E0B' }} />
                <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)' }}>Store Details & Location</span>
                {fieldsLocked && (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
              <div>
                ...
                <input type="text" value={pharmacyName} onChange={(e) => setPharmacyName(e.target.value)} disabled={fieldsLocked} style={fieldsLocked ? lockedInputStyle : inputStyle} required />
              </div>
              <div>
                <label style={labelStyle}>Drug License / Registration No.</label>
                <input type="text" value={initialProfile.registrationNumber} disabled style={lockedInputStyle} title="Registration number cannot be changed" />
              </div>
            </div>
            ...
            <div style={{ background: 'var(--table-header-bg)', border: '1px solid var(--card-border)', borderRadius: 10, padding: 14 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>Store Location (GPS)</span>
                </div>
                {!fieldsLocked && (
                  <button type="button" onClick={() => setShowMapModal(true)}
                    style={{ ...btnStyle, background: '#3B82F6', fontSize: 12, padding: '7px 14px' }}>
              ...
              {latitude && longitude && (
                <a href={`https://www.google.com/maps?q=${latitude},${longitude}`} target="_blank" rel="noopener noreferrer"
                  style={{ fontSize: 11, color: '#3B82F6', fontWeight: 600, textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: 4, marginTop: 8 }}>
            ...
            {!fieldsLocked && (
              <div style={{ borderTop: '1px solid var(--card-border)', paddingTop: 16 }}>
                <span style={labelStyle}>Custom / Dynamic Fields</span>
                ...
                {showAddFieldForm ? (
                  ...
                  <button type="button" onClick={() => setShowAddFieldForm(true)} style={{ ...btnStyle, background: 'var(--table-header-bg)', color: 'var(--text-secondary)', border: '1px solid var(--card-border)', marginTop: 10, fontSize: 12 }}>
                    <Plus style={{ width: 13, height: 13 }} /> Add Custom Field
                  </button>
                )}
              </div>
            )}
            ...
            <div style={{ borderTop: '1px solid var(--card-border)', paddingTop: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12, flexWrap: 'wrap', gap: 8 }}>
                <div>
                  ...
                  <p style={{ fontSize: 11, color: 'var(--text-secondary)', margin: 0 }}>
                    DDA license, VAT/PAN certificate, citizenship, store photos (Max 500MB each, up to 10 images)
                  </p>
                </div>
              ...
              {/* Upload grid */}
              <input type="file" accept="image/*" multiple id="retailer-doc-upload-input" style={{ display: 'none' }}
                onChange={async (e) => {
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                ...
                {!fieldsLocked && registrationImages.length < 10 && (
                  <label htmlFor="retailer-doc-upload-input"
                    style={{ width: 72, height: 72, borderRadius: 8, border: '2px dashed #F59E0B', background: 'rgba(245,158,11,0.06)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#F59E0B', fontSize: 10, fontWeight: 700, textAlign: 'center', gap: 4, flexShrink: 0 }}>
      ...
      {activeTab === 'security' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div style={{ background: 'var(--card-bg)', borderRadius: 10, border: '1px solid var(--card-border)', padding: 20 }}>
            ...
            <select value={inactivityTimeout} onChange={(e) => handleSaveSecurity(e.target.value)} style={{ ...inputStyle, marginTop: 12 }}>
      ...
      {activeTab === 'theme' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div style={{ background: 'var(--card-bg)', borderRadius: 10, border: '1px solid var(--card-border)', padding: 20 }}>
            ...
            <p style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 4, marginBottom: 14 }}>Choose between light and dark mode for the retailer dashboard.</p>
            <div style={{ display: 'flex', gap: 12 }}>
              <button type="button" onClick={() => handleToggleTheme('light')} style={{
                flex: 1, padding: '14px 10px', borderRadius: 10,
                border: themeMode === 'light' ? '2px solid #F59E0B' : '1px solid var(--card-border)',
                background: themeMode === 'light' ? '#FFFBEB' : 'var(--card-bg)',
                color: themeMode === 'light' ? '#D97706' : 'var(--text-secondary)',
                ...
                Light Mode
                {themeMode === 'light' && <span style={{ fontSize: 10, background: '#F59E0B', color: '#fff', padding: '2px 8px', borderRadius: 10 }}>ACTIVE</span>}
              </button>
              <button type="button" onClick={() => handleToggleTheme('dark')} style={{
                flex: 1, padding: '14px 10px', borderRadius: 10,
                border: themeMode === 'dark' ? '2px solid #F59E0B' : '1px solid var(--card-border)',
                ...
                Dark Mode
                {themeMode === 'dark' && <span style={{ fontSize: 10, background: '#F59E0B', color: '#fff', padding: '2px 8px', borderRadius: 10 }}>ACTIVE</span>}
              </button>
            </div>
          </div>
          <div style={{ background: 'var(--card-bg)', borderRadius: 10, border: '1px solid var(--card-border)', padding: 20 }}>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              {([
                ...
                <button key={s.key} type="button" onClick={() => handleToggleFontScale(s.key)} style={{
                  flex: 1, padding: '14px 10px', borderRadius: 10,
                  border: fontScale === s.key ? '2px solid #F59E0B' : '1px solid var(--card-border)',
                  background: fontScale === s.key ? '#FFFBEB' : 'var(--card-bg)',
                  color: fontScale === s.key ? '#D97706' : 'var(--text-secondary)',
                  ...
                  <span style={{ fontSize: 10, color: fontScale === s.key ? '#D97706' : 'var(--text-muted)', textAlign: 'center' }}>{s.desc}</span>
                  {fontScale === s.key && <span style={{ fontSize: 10, background: '#F59E0B', color: '#fff', padding: '2px 8px', borderRadius: 10, marginTop: 2 }}>ACTIVE</span>}
                </button>
              ))}
            </div>
          </div>
      ...
      {showStaffModal && (
        <Modal onClose={() => setShowStaffModal(false)} title={editingStaff ? 'Update Employee Permissions' : 'Register New Shop Employee'}>
          <form onSubmit={handleSaveStaff} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div><label style={labelStyle}>Email Address (Login ID)</label><input type="email" required disabled={!!editingStaff} value={staffEmail} onChange={(e) => setStaffEmail(e.target.value)} placeholder="e.g. ram@pharmacy.com" style={{ ...inputStyle, background: editingStaff ? 'var(--table-header-bg)' : 'var(--card-bg)' }} /></div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, background: 'var(--table-header-bg)', padding: 10, borderRadius: 8, border: '1px solid var(--card-border)' }}>
                {AVAILABLE_FEATURES.map((feat) => (
                  <label key={feat} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--text-secondary)', cursor: 'pointer' }}>
                    <input type="checkbox" checked={staffFeatures.includes(feat)} onChange={() => toggleFeature(feat)} style={{ cursor: 'pointer' }} />{feat}
                  </label>
                ))}
              </div>
            </div>
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 400, background: 'rgba(15,23,42,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ width: '100%', maxWidth: 720, background: 'var(--card-bg)', borderRadius: 16, border: '1px solid var(--card-border)', display: 'flex', flexDirection: 'column', maxHeight: '92vh', overflow: 'hidden', boxShadow: '0 32px 64px rgba(0,0,0,0.3)' }}>
            ...
            <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--card-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--table-header-bg)', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <MapPin style={{ width: 16, height: 16, color: '#3B82F6' }} />
                <span style={{ fontSize: 15, fontWeight: 800 }}>Set Store Location from Map</span>
              </div>
            ...
            <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--card-border)', background: 'var(--card-bg)', flexShrink: 0 }}>
              <div style={{ display: 'flex', gap: 8 }}>
                <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8, background: 'var(--table-header-bg)', border: '1px solid var(--card-border)', borderRadius: 8, padding: '8px 12px' }}>
                  ...
                  <input
                    type="text"
                    placeholder="Search for a place, address or pharmacy..."
            ...
            <div style={{ padding: '12px 20px', borderTop: '1px solid var(--card-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0, background: 'var(--table-header-bg)', gap: 12, flexWrap: 'wrap' }}>
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button onClick={() => { setShowMapModal(false); setMapSearchResults([]); }} style={{ ...btnStyle, background: 'var(--card-bg)', color: 'var(--text-secondary)', border: '1px solid var(--card-border)', fontSize: 13 }}>
                  Cancel
                </button>
                <button onClick={handleConfirmMapLocation} style={{ ...btnStyle, background: '#10B981', fontSize: 13 }}>
                  <Check style={{ width: 14, height: 14 }} /> Confirm Location
                </button>
              </div>
            </div>
          </div>
          ...
          <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 680, background: 'var(--card-bg)', borderRadius: 16, border: '1px solid var(--card-border)', display: 'flex', flexDirection: 'column', maxHeight: '94vh', overflow: 'hidden', boxShadow: '0 32px 64px rgba(0,0,0,0.5)' }}>
            ...
            <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--card-border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'var(--table-header-bg)', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 800 }}>Registration Documents</span>
