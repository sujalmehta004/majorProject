  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24, padding: '24px 0' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 16 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 900, color: 'var(--text-primary)', margin: 0 }}>Pharmacy Inventory</h1>
          <p style={{ fontSize: 14, color: 'var(--text-secondary)', margin: '4px 0 0' }}>
            Shelf stock overview — Click any row for advanced batch details
            <span style={{ marginLeft: 10, fontSize: 11, color: 'var(--text-muted)' }}>[ / ] Search · [ Ctrl+N ] Intake Batch</span>
          </p>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <button
            onClick={() => setShowBatchPrintModal(true)}
            style={{ background: '#10B981', color: '#FFFFFF', display: 'flex', alignItems: 'center', gap: 8, padding: '10px 18px', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: 'pointer', border: 'none', boxShadow: '0 4px 12px rgba(16,185,129,0.3)' }}
          >
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        {[
          ...
          <div key={s.label} style={{ background: s.bg, border: `1.5px solid ${s.border}`, borderRadius: 12, padding: '12px 20px', display: 'flex', gap: 12, alignItems: 'center' }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>{s.label}</span>
          </div>
        ))}
      </div>

      {/* ── Filters bar ── */}
      <div style={{ display: 'flex', gap: 12, padding: '14px 18px', background: 'var(--card-bg)', borderRadius: 14, border: '1px solid var(--card-border)', flexWrap: 'wrap' }}>
        ...
        <div style={{ fontSize: 14, color: 'var(--text-muted)', alignSelf: 'center', fontWeight: 600, whiteSpace: 'nowrap' }}>
      ...
      <div style={{ background: 'var(--card-bg)', borderRadius: 16, border: '1px solid var(--card-border)', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.03)' }}>
        {loading ? (
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '80px', flexDirection: 'column', gap: 12 }}>
            ...
            <span style={{ fontSize: 14, color: 'var(--text-muted)' }}>Loading inventory…</span>
          </div>
        ) : filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '80px', color: 'var(--text-muted)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ fontSize: 14 }}>Try adjusting filters or record a manual intake batch</div>
          </div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left', fontSize: 14 }}>
            <thead>
              <tr style={{ background: 'var(--table-header-bg)', borderBottom: '2px solid #F1F5F9' }}>
                <th style={{ padding: '14px 20px', color: 'var(--text-secondary)', fontWeight: 700, cursor: 'pointer', userSelect: 'none' }} onClick={() => handleSort('name')}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>Medicine <SortIcon k="name" /></span>
                </th>
                <th style={{ padding: '14px 20px', color: 'var(--text-secondary)', fontWeight: 700 }}>SKU / Category</th>
                <th style={{ padding: '14px 20px', color: 'var(--text-secondary)', fontWeight: 700, cursor: 'pointer', userSelect: 'none' }} onClick={() => handleSort('batch')}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>Batch <SortIcon k="batch" /></span>
                </th>
                <th style={{ padding: '14px 20px', color: 'var(--text-secondary)', fontWeight: 700, cursor: 'pointer', userSelect: 'none' }} onClick={() => handleSort('quantity')}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>Stock <SortIcon k="quantity" /></span>
                </th>
                <th style={{ padding: '14px 20px', color: 'var(--text-secondary)', fontWeight: 700 }}>Buying Price</th>
                <th style={{ padding: '14px 20px', color: 'var(--text-secondary)', fontWeight: 700 }}>Selling Price</th>
                <th style={{ padding: '14px 20px', color: 'var(--text-secondary)', fontWeight: 700 }}>Rack</th>
                <th style={{ padding: '14px 20px', color: 'var(--text-secondary)', fontWeight: 700, cursor: 'pointer', userSelect: 'none' }} onClick={() => handleSort('expiry')}>
                ...
                return (
                  ...
                  >
                    <td style={{ padding: '16px 20px' }}>
                      <div style={{ fontWeight: 800, color: 'var(--text-primary)' }}>{item.product.name}</div>
                    ...
                    <td style={{ padding: '16px 20px' }}>
                      <span style={{ fontFamily: 'monospace', fontSize: 14, color: 'var(--text-secondary)', background: 'var(--table-header-bg)', padding: '2px 6px', borderRadius: 4 }}>{item.product.sku}</span>
                    ...
                    <td style={{ padding: '16px 20px' }}>
                      <div style={{ fontWeight: 800, color: 'var(--text-primary)' }}>{item.quantity.toLocaleString()} <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)' }}>units</span></div>
                    ...
                    <td style={{ padding: '16px 20px' }}>
                      <div style={{ fontWeight: 800, color: 'var(--text-secondary)', fontFamily: 'monospace' }}>Rs. {item.buyingPrice?.toFixed(2) || '0.00'}</div>
                      <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>per box</div>
                    </td>
                    <td style={{ padding: '16px 20px' }}>
                    ...
                    <td style={{ padding: '16px 20px' }}>
                      <div style={{ fontWeight: 700, color: 'var(--text-secondary)', background: item.rack ? '#F1F5F9' : 'transparent', padding: item.rack ? '2px 8px' : 0, borderRadius: 6, display: 'inline-block', fontSize: 14 }}>{item.rack || '—'}</div>
                    </td>
                    <td style={{ padding: '16px 20px' }}>
                    ...
                    <td style={{ padding: '16px 20px', textAlign: 'center' }} onClick={(e) => e.stopPropagation()}>
                      <div style={{ display: 'flex', justifyContent: 'center', gap: 10 }}>
                        <button
                          ...
                          style={{ background: 'var(--table-header-bg)', border: '1px solid var(--card-border)', borderRadius: 6, padding: '6px', color: 'var(--text-secondary)', cursor: 'pointer', display: 'flex', alignItems: 'center' }}
                        ...
                        <button
                          ...
                          style={{ background: 'var(--table-header-bg)', border: '1px solid var(--card-border)', borderRadius: 6, padding: '6px', color: '#3B82F6', cursor: 'pointer', display: 'flex', alignItems: 'center' }}
                          title="Edit quantity"
                        >
      ...
      {detailBatch && (() => {
        ...
        return (
          <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(15,23,42,0.35)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
            <div style={{ width: '100%', maxWidth: 520, background: 'var(--card-bg)', borderRadius: 20, border: '1px solid var(--card-border)', overflow: 'hidden', boxShadow: 'none' }}>
              {/* Modal Header */}
              <div style={{ padding: '22px 24px', background: 'linear-gradient(135deg, #F59E0B08, transparent)', borderBottom: '1px solid #F1F5F9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div>
                    <h3 style={{ fontSize: 16, fontWeight: 900, color: 'var(--text-primary)', margin: 0 }}>{detailBatch.product.name}</h3>
                    <p style={{ fontSize: 14, color: 'var(--text-muted)', margin: 0 }}>Batch {detailBatch.batchNumber}</p>
                  </div>
                </div>
                <button onClick={() => setDetailBatch(null)} style={{ background: 'var(--table-header-bg)', border: '1px solid var(--card-border)', borderRadius: 8, padding: '6px', cursor: 'pointer', color: 'var(--text-secondary)' }}>
              ...
              <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 8, background: expiryStatus.bg, border: `1px solid ${expiryStatus.color}30` }}>
                  <span style={{ fontSize: 14, fontWeight: 800, color: expiryStatus.color }}>⬤ {expiryStatus.label}</span>
                  <span style={{ fontSize: 14, color: 'var(--text-secondary)' }}>— expires {new Date(detailBatch.expiryDate).toLocaleDateString()}</span>
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  {[
                    ...
                    return (
                      <div key={row.label} style={{ background: 'var(--table-header-bg)', borderRadius: 10, padding: '12px 14px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 4 }}>
                          <RowIcon style={{ width: 11, height: 11, color: 'var(--text-muted)' }} />
                          <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase' }}>{row.label}</span>
                        </div>
                        <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)', fontFamily: row.mono ? 'monospace' : undefined }}>{row.val}</div>
                      </div>
                    );
                  })}
                </div>
                ...
                <div style={{ background: 'var(--table-header-bg)', borderRadius: 12, padding: 16 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: 12 }}>Stock Breakdown</div>
                  <div style={{ display: 'flex', gap: 8 }}>
                    {[
                      { val: boxes, label: 'Boxes', color: '#3B82F6' },
                      { val: strips, label: 'Strips', color: '#F59E0B' },
                      { val: tablets, label: 'Tablets', color: '#10B981' },
                      { val: detailBatch.quantity, label: 'Total Units', color: 'var(--text-secondary)' },
                    ].map((s) => (
                      <div key={s.label} style={{ flex: 1, textAlign: 'center', background: 'var(--card-bg)', borderRadius: 8, padding: '10px 8px', border: `1px solid ${s.color}20` }}>
                        <div style={{ fontSize: 18, fontWeight: 900, color: s.color }}>{s.val}</div>
                        <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', marginTop: 2 }}>{s.label}</div>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div style={{ padding: '16px 24px', borderTop: '1px solid #F1F5F9', display: 'flex', gap: 10 }}>
                <button
                  ...
                  style={{ flex: 1, padding: '11px', borderRadius: 10, border: '1px solid var(--card-border)', fontSize: 14, fontWeight: 700, cursor: 'pointer', background: 'var(--card-bg)', color: 'var(--text-secondary)' }}
                >
                  Edit Quantity
                </button>
                <button
                  onClick={() => { setPrintPreviewBatch(detailBatch); setDetailBatch(null); }}
                  style={{ flex: 2, padding: '11px', borderRadius: 10, border: 'none', fontSize: 14, fontWeight: 800, cursor: 'pointer', background: '#F59E0B', color: '#FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
                >
                  <Printer style={{ width: 15, height: 15 }} />
      ...
      {showBatchPrintModal && (() => {
        ...
        return (
          <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(15,23,42,0.35)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
            <div style={{ width: '100%', maxWidth: 650, background: 'var(--card-bg)', borderRadius: 20, border: '1px solid var(--card-border)', padding: 28, display: 'flex', flexDirection: 'column', gap: 18, boxShadow: 'none', maxHeight: '90vh' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button onClick={() => setShowBatchPrintModal(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)' }}>
                  <X style={{ width: 18, height: 18 }} />
                </button>
              </div>

              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Select Batch Number</label>
                <select
                  value={selectedBatchNumber}
                  onChange={(e) => setSelectedBatchNumber(e.target.value)}
                  style={{ padding: '10px', borderRadius: 8, border: '1px solid var(--card-border)', outline: 'none', fontSize: 14, background: 'var(--card-bg)' }}
                >
              ...
              {selectedBatchNumber && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10, flex: 1, overflowY: 'auto' }}>
                  <div style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Medicines in Batch "{selectedBatchNumber}"</div>
                  
                  <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
                    <thead>
                      <tr style={{ background: 'var(--table-header-bg)', borderBottom: '1px solid #E2E8F0', textAlign: 'left' }}>
                        ...
                        <tr key={item.id} style={{ borderBottom: '1px solid #F1F5F9' }}>
                          ...
                          <td style={{ padding: '8px 10px' }}>
                            <input
                              ...
                              style={{ width: '100%', padding: '4px 8px', borderRadius: 6, border: '1px solid var(--card-border)', outline: 'none' }}
                            />
                          </td>
                        </tr>
                      ))}
              ...
              <div style={{ display: 'flex', gap: 12, marginTop: 12 }}>
                <button onClick={() => setShowBatchPrintModal(false)} style={{ flex: 1, padding: '12px', borderRadius: 10, border: '1px solid var(--card-border)', fontSize: 14, fontWeight: 700, cursor: 'pointer', background: 'var(--card-bg)', color: 'var(--text-secondary)' }}>
                  Cancel
                </button>
                <button
                  onClick={onPrintBatchClick}
                  disabled={!selectedBatchNumber || totalBatchPrintCount === 0}
                  style={{ flex: 2, padding: '12px', borderRadius: 10, border: 'none', fontSize: 14, fontWeight: 800, cursor: selectedBatchNumber && totalBatchPrintCount > 0 ? 'pointer' : 'not-allowed', background: selectedBatchNumber && totalBatchPrintCount > 0 ? '#10B981' : '#CBD5E1', color: '#FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}
                >
                  <Printer style={{ width: 15, height: 15 }} />
      ...
      {printPreviewBatch && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(15,23,42,0.35)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ width: '100%', maxWidth: 400, background: 'var(--card-bg)', borderRadius: 20, border: '1px solid var(--card-border)', padding: 28, display: 'flex', flexDirection: 'column', gap: 18, boxShadow: 'none' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <h3 style={{ fontSize: 16, fontWeight: 900, color: 'var(--text-primary)', margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ background: 'var(--table-header-bg)', borderRadius: 12, padding: 14 }}>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 700, marginBottom: 4 }}>PRINTING FOR</div>
              <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--text-primary)' }}>{printPreviewBatch.product.name}</div>
              <div style={{ fontSize: 14, color: 'var(--text-secondary)', marginTop: 2 }}>Batch: {printPreviewBatch.batchNumber} · Expiry: {new Date(printPreviewBatch.expiryDate).toLocaleDateString()}</div>
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Number of Copies</label>
              <div style={{ display: 'flex', gap: 8 }}>
                {[1, 5, 10, 25].map((n) => (
                  <button
                    key={n}
                    onClick={() => setPrintCopies(n)}
                    style={{ flex: 1, padding: '8px', borderRadius: 8, border: `1.5px solid ${printCopies === n ? '#F59E0B' : '#E2E8F0'}`, background: printCopies === n ? 'rgba(245,158,11,0.08)' : '#FFFFFF', fontSize: 14, fontWeight: 700, cursor: 'pointer', color: printCopies === n ? '#F59E0B' : '#475569' }}
                  >
                    {n}
                  </button>
                ))}
              ...
              <input
                ...
                style={{ padding: '10px', borderRadius: 8, border: '1px solid var(--card-border)', outline: 'none', fontSize: 14 }}
              />
            </div>

            <div style={{ display: 'flex', gap: 12, marginTop: 4 }}>
              <button onClick={() => setPrintPreviewBatch(null)} style={{ flex: 1, padding: '12px', borderRadius: 10, border: '1px solid var(--card-border)', fontSize: 14, fontWeight: 700, cursor: 'pointer', background: 'var(--card-bg)', color: 'var(--text-secondary)' }}>
                Cancel
              </button>
              <button onClick={() => handlePrintLabels(printPreviewBatch, printCopies)} style={{ flex: 2, padding: '12px', borderRadius: 10, border: 'none', fontSize: 14, fontWeight: 800, cursor: 'pointer', background: '#F59E0B', color: '#FFFFFF', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
                <Printer style={{ width: 15, height: 15 }} />
      ...
      {showAddModal && (
        <div style={{ position: 'fixed', left: 0, top: 0, width: '100vw', height: '100vh', zIndex: 200, background: 'rgba(15,23,42,0.45)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
          <form
            ...
            style={{ width: '100%', maxWidth: 760, maxHeight: '85vh', background: 'var(--card-bg)', borderRadius: 20, border: '1px solid var(--card-border)', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: 'none', margin: 'auto' }}
          >
            {/* Header */}
            <div style={{ padding: '24px 40px', borderBottom: '1px solid #F1F5F9', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'linear-gradient(135deg, rgba(245,158,11,0.03), transparent)', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                ...
                <div>
                  <h3 style={{ fontSize: 22, fontWeight: 900, color: 'var(--text-primary)', margin: 0 }}>Intake & Register Medicine Batch</h3>
                  <p style={{ fontSize: 14, color: 'var(--text-secondary)', margin: '4px 0 0' }}>Register details of newly received medicine stock inside the warehouse console</p>
                </div>
              </div>
              <button type="button" onClick={() => setShowAddModal(false)} style={{ background: 'var(--table-header-bg)', border: '1px solid var(--card-border)', borderRadius: 12, padding: '8px', cursor: 'pointer', color: 'var(--text-secondary)' }}>
            ...
            <div style={{ padding: '24px 40px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 24, flex: 1, width: '100%' }}>
              ...
              <div style={{ display: 'flex', gap: 10, background: 'var(--table-header-bg)', padding: 6, borderRadius: 14, border: '1px solid var(--card-border)' }}>
                <button
                  type="button"
              ...
              {intakeMode === 'select' ? (
                /* Select Dropdown */
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <label style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Select Medicine Profile</label>
                  <select
                    value={selectedProdId}
                    onChange={(e) => setSelectedProdId(e.target.value)}
                    style={{ padding: '14px', borderRadius: 12, border: '1px solid var(--card-border)', outline: 'none', fontSize: 14, background: 'var(--card-bg)' }}
                  >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 20, border: '1.5px solid #FEF3C7', padding: 24, borderRadius: 20, background: '#FFFDF5' }}>
                  <div style={{ fontSize: 14, fontWeight: 900, color: '#D97706', display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      <label style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Medicine Name (Generic)</label>
                      <input
                        type="text"
                        value={newProdName}
                        onChange={(e) => setNewProdName(e.target.value)}
                        placeholder="e.g. Paracetamol 500mg"
                        style={{ padding: '12px 14px', borderRadius: 12, border: '1px solid var(--card-border)', outline: 'none', fontSize: 14, background: 'var(--card-bg)' }}
                      />
                    </div>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      <label style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Unique SKU code</label>
                      <input
                        type="text"
                        value={newProdSku}
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr 1fr', gap: 14 }}>
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      <label style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Category</label>
                      <div style={{ display: 'flex', gap: 8 }}>
                        <select
                          ...
                          style={{ flex: 1, padding: '12px 14px', borderRadius: 12, border: '1px solid var(--card-border)', outline: 'none', fontSize: 14, background: 'var(--card-bg)' }}
                        >
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      <label style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Tablets / Strip</label>
                      <input
                        type="number"
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      <label style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Strips / Box</label>
                      <input
                        type="number"
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                <div style={{ fontSize: 14, fontWeight: 900, color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <label style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Batch Number</label>
                    <input
                      type="text"
                      value={batchNumber}
                      onChange={(e) => setBatchNumber(e.target.value)}
                      placeholder="e.g. B-PARA-2026"
                      style={{ padding: '12px 14px', borderRadius: 12, border: '1px solid var(--card-border)', outline: 'none', fontSize: 14 }}
                    />
                  </div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <label style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Expiry Date</label>
                    <input
                      type="date"
                      value={expiryDate}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <label style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Rack Number / Location</label>
                    <input
                      type="text"
                      value={rack}
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 16 }}>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <label style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Quantity</label>
                    <input
                      type="number"
                      value={quantity}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    <label style={{ fontSize: 11, fontWeight: 800, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Tracking Unit (UOM)</label>
                    <select
                      value={intakeUom}
            ...
            {/* Footer */}
            <div style={{ padding: '24px 40px', borderTop: '1px solid #F1F5F9', display: 'flex', justifyContent: 'flex-end', gap: 14, background: 'var(--table-header-bg)', flexShrink: 0 }}>
              <button type="button" onClick={() => setShowAddModal(false)} style={{ padding: '12px 24px', borderRadius: 12, border: '1px solid var(--card-border)', fontSize: 14, fontWeight: 700, cursor: 'pointer', background: 'var(--card-bg)', color: 'var(--text-secondary)' }}>
                Cancel
              </button>
      ...
      {editingItem && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, background: 'rgba(15,23,42,0.35)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <form
            ...
            style={{ width: '100%', maxWidth: 380, background: 'var(--card-bg)', borderRadius: 20, border: '1px solid var(--card-border)', overflow: 'hidden', boxShadow: 'none' }}
          >
            <div style={{ padding: '22px 24px', borderBottom: '1px solid #F1F5F9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button type="button" onClick={() => setEditingItem(null)} style={{ background: 'var(--table-header-bg)', border: '1px solid var(--card-border)', borderRadius: 8, padding: '6px', cursor: 'pointer', color: 'var(--text-secondary)' }}>
                <X style={{ width: 16, height: 16 }} />
              </button>
            </div>
            <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ background: 'var(--table-header-bg)', borderRadius: 10, padding: '12px 14px' }}>
                <div style={{ fontSize: 14, fontWeight: 800, color: 'var(--text-primary)' }}>{editingItem.product.name}</div>
                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>Batch: {editingItem.batchNumber}</div>
                <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>Current: {editingItem.quantity.toLocaleString()} units</div>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>New Quantity (Base Units / Tablets)</label>
                <input
                  type="number"
                  value={editQty}
                  onChange={(e) => setEditQty(e.target.value)}
                  autoFocus
                  style={{ padding: '12px', borderRadius: 8, border: '1px solid var(--card-border)', outline: 'none', fontSize: 18, fontWeight: 700, textAlign: 'center' }}
                />
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Buying Price (Rs. / Box)</label>
                <input
                  type="number"
                  step="0.01"
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Selling Price (Rs. / Box)</label>
                <input
                  type="number"
                  step="0.01"
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                <label style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase' }}>Rack Number / Location</label>
                <input
                  type="text"
                  value={editRack}
            ...
            <div style={{ padding: '16px 24px', borderTop: '1px solid #F1F5F9', display: 'flex', gap: 10 }}>
              <button type="button" onClick={() => setEditingItem(null)} style={{ flex: 1, padding: '11px', borderRadius: 10, border: '1px solid var(--card-border)', fontSize: 14, fontWeight: 700, cursor: 'pointer', background: 'var(--card-bg)', color: 'var(--text-secondary)' }}>
                Cancel
              </button>
              <button type="submit" style={{ flex: 2, padding: '11px', borderRadius: 10, border: 'none', fontSize: 14, fontWeight: 800, cursor: 'pointer', background: '#3B82F6', color: '#FFFFFF' }}>
                Save Changes
              </button>
            </div>
          </form>
